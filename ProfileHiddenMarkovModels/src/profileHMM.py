...
while i < num_of_chars + 1:
    ...
    if i in matchStatePossition:  # if match state
        ...
        if i != 0:
            if transition_probability[D]['strings']:  # D --> D and D --> M
                ...
                if d_deleteList: # if d_deleteLİst not empty
                    transition_probability[D][nextD] = {'prob' : float(len(d_deleteList) / len(transition_probability[D]['strings'])),
                                                        'strings' : d_deleteList} # D --> D
                    transition_probability[nextD]['strings'].extend(d_deleteList)
                if d_matchList: # if d_matchList not empty
                    transition_probability[D][nextM] = {'prob' : float(len(d_matchList) / len(transition_probability[D]['strings'])),
            ...
            if transition_probability[I]['strings']:  # I --> M and I --> D
                ...
                try:
                    i_deleteList = list(set([n for n in transition_probability[I]['strings'] if inputString[n][i] == '.']))
                except Exception: 
                    pass
                i_matchList = list(set([n for n in transition_probability[I]['strings'] if n not in i_deleteList]))
                if i_deleteList: # if i_deleteList not empty
                    transition_probability[I][nextD] = {'prob' : float(len(i_deleteList) / len(transition_probability[I]['strings'])),
                ...
                if i_matchList:  # if i_matchList not empty
                    transition_probability[I][nextM] = {'prob' : float(len(i_matchList) / len(transition_probability[I]['strings'])),
        ...
        if transition_probability[M]['strings']:  # M --> D and M --> M
            ...
            try:
                m_deleteList = [n for n in transition_probability[M]['strings'] if inputString[n][i] == '.']
            except Exception:
                pass
            m_matchList  = [n for n in transition_probability[M]['strings'] if n not in m_deleteList + transition_probability[I]['strings']]
            if m_deleteList: # if m_deleteList not empty
                transition_probability[M][nextD] = {'prob' : float(len(m_deleteList) / len(transition_probability[M]['strings'])),
                                                    'strings' : m_deleteList} # M --> D
                transition_probability[nextD]['strings'].extend(m_deleteList)
            if m_matchList: # if m_matchList not empty
                transition_probability[M][nextM] = {'prob' :  float(len(m_matchList) / len(transition_probability[M]['strings'])),
        ...
        while True: # loop for next insert state
            insert_state_list.extend([n for n in range(num_of_strings) if inputString[n][i] != '.'])
        ...
        if insert_state_list: # if insert_state not empty
            come_from_match  = [n for n in transition_probability[M]['strings'] if n in insert_state_list]  # M --> I
            come_from_del    = [n for n in transition_probability[D]['strings'] if n in insert_state_list]  # D --> I
            come_from_insert = [n for n in insert_state_list if insert_state_list.count(n) > 1]             # I --> I
            if come_from_match:  # if string come from match state
                transition_probability[M][I] = {'prob' :  float(len(come_from_match) / len(transition_probability[M]['strings'])),
                                                        'strings' : come_from_match} # M(j) --> I(j)
            if come_from_del:    # if string come from delete state
                transition_probability[D][I] = {'prob' :  float(len(come_from_del) / len(transition_probability[D]['strings'])),
                                                        'strings' : come_from_del}   # D(j) --> I(j)
            if come_from_insert: # if string come from insert state
                transition_probability[I][I] = {'prob' :  float((len(insert_state_list) - len(come_from_insert)) / len(insert_state_list)),
                                                        'strings' : list(set(come_from_insert))}
            transition_probability[I]['strings'].extend(insert_state_list)
    emission_probability[nextM] = {n : frequencyList[i][n]  for n in frequencyList[i] if n != '.'} # get emission probability without '.'
    i += 1

# write emission and transition probability
e = list(emission_probability.items())
t = list(transition_probability.items())
t.sort()
e.sort()
print(*e, sep="\n", file=open("e.out", "w")) 
print(*t, sep="\n", file=open("t.out", "w")) 
